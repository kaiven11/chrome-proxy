  ...
  function loadCredentials() {
    chrome.runtime.sendMessage({ action: 'getCredentials' }, function(response) {
      if (response && response.credentials) {
        ...
        if (response.credentials.username === '6075849-a11a16ce') {
          showStatus('已加载默认代理认证信息', 'success');
        } else {
          showStatus('已加载保存的凭据', 'info');
        }
      } else {
        // 如果没有凭据，显示默认信息
        usernameInput.value = '6075849-a11a16ce';
        passwordInput.value = 'e9a6377b-global-96417862-5m';
        showStatus('已加载默认代理认证信息', 'info');
      }
    });
  }
